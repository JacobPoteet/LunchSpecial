...
worker/service.ts     PURE per-day folds (UTC hour buckets → ET hour profile, all-kinds totals, pace baseline) — unit tested
...
- **Repeat visits** (admin Trends tab): the restaurant read on top of new-vs-returning — of the players who have visited N times, how many came back for an N+1th? A **visit is an ET day the device played on**, not a round (four leftovers in one sitting is one visit), which is why the fold reuses the same `activeDaysByPlayer` sets `foldPlayerActivity` already builds. All pure in `worker/players.ts` (`foldRetention`, unit-tested in players.test.ts), rendered by `RetentionCurve` in TrendsPanel.tsx. All-time and surface-filtered; **always folded against the real `today`, never the picked day** — the window asks "has enough time passed by now", and answering it from a past day would call every visit since then a no-show. Four things are load-bearing:
...
- **Growth chart** (admin Trends tab, the first panel): the **running total** of games played per ET day since the first round ever recorded, every kind together, with a straight constant-pace line through it (GitHub #90). It rides `AnalyticsSummary.growth` and is folded from the **same all-time hour buckets** `hourly`/`activeDates` already read, so it costs no extra query — pure in `worker/growth.ts` (`foldGrowth`, unit-tested in growth.test.ts), drawn by `GrowthChart` in TrendsPanel.tsx. Six things are load-bearing:
