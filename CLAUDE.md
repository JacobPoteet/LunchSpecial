...
- **`?demo=1` always lands you in the demo, replacing a full session if you had one.** The rule this replaced said the opposite, and it sounded protective while being the exact reverse: `/admin?demo=1` has two visitors, a stranger following the link and the owner checking what that stranger sees, and the owner is the one with a cookie. So the owner was the only person the preview never showed the demo to — full back office, every delete live, nothing on screen saying so. **A preview that shows you something other than what it is previewing is worse than no preview.** It swaps the cookie rather than drawing a read-only skin over a session that can still write, so what you see is what a visitor gets. Previewing costs you the full session; the banner says so and the nav carries a Clock in button. `demoSessionNeeded` is the fold, and it has the regression test.
