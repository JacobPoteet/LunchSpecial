...
src/game/             GamePage (orchestrator), components.tsx (Modal/GuessRow/ClueTicket/GuessInput/
                      ...
                      BuildTag.tsx (the always-on build marker, one span in the footer)
...
- **The build marker.** Every page of the game carries the version it was built from in the footer line beside the byline (`Created by Jacob Poteet · v1.7.0`, a trailing `*` for a dirty tree), so a screenshot or a screen recording says for itself which build it happened on. **Always on, everywhere, including production** — a marker you have to remember to switch on is one that isn't in the shot you needed it in. Four things hold: (1) it is **in the page flow, never a fixed badge** — the fixed bottom-right pill it started as covered the bottom of the check on small iPhones and ate a third of the footer's width; (2) the player sees the **version, not the sha** (`buildVersion`); the full sha rides on the span's `title` and on the admin line, because a release tag resolves to one commit and the admin is where "did that deploy land" gets asked; (3) it is **not a round mode and not a setting** — nothing about it reaches the server, the beacons or localStorage, and there is no toggle to plumb; (4) the admin carries the full label (`v1.7.0 · c61d712`) as a footer at the bottom of every screen, never in the nav, which wraps on a narrow window and put it somewhere different on each panel
