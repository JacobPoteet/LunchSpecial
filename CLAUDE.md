...
- **Never `--ink` on a `--mustard` fill; the text on mustard is `--on-near`.** Daytime the two are the same hex, which is how four rules (the highlighted order-bar option, the fan seal, the promoted suggest button, the archive's hovered day) shipped with `--ink` and nobody saw a difference until the bar's pale-cream ink landed on the autocomplete at 1.9:1 (#179). The a11y scan now measures the bar's list with a row highlighted, because every earlier state typed and clicked through in one move and the highlight was never on screen when axe looked.
- **A hover fill is `--cherry-hover`, never `--cherry-dark`.** Same lesson from the other end: not a stray hex but a token reused in the wrong *direction*. Deepening cherry on hover is a daytime idea; at the bar `--cherry-dark` is the old daytime cherry, so every cherry hover painted `--on-cherry` at 3.44. The night value goes **up** (`#ff8496`, 7.6 on `--on-cherry`) because a sign brightens when you reach for it. `--cherry-dark` keeps the jobs with nothing on top of it, the card frame and the button lip.
...
- **`play_date` is never joined against `schedule`, and a Nightcap is never named from a dish.** The night key and an ET day are the same shape, so the join succeeds and returns the lunch Special booked for that calendar date — which is exactly how every Nightcap in the activity feed came out labelled with a dish nobody at the bar played. A Nightcap's target comes off `drink_id` and the `drinks` table, the only place it was ever written, and the feed links it nowhere: the dish report reports on dishes.
- **A list of the kinds is written off `ROUND_KINDS` or `KIND_META`, never typed out.** Three hardcoded kinds is how the device-review breakdown counted Nightcaps in its total and left them out of the sentence underneath. Same reason `sumKinds` is a reduce over the enum.
- **A guess count printed beside another kind's carries its ceiling** (`maxGuessesFor`). The activity log is the one screen where a four-guess round and a six-guess round sit one row apart, so "Solved in 3" alone is ambiguous there in a way it is nowhere else.
- **Nightcaps are excluded from the dish report outright**, not left to fall through it. They carry no `dish_id`, so an unfiltered query drops every one of them into `untracked`, which the panel prints as "rounds that predate dish tracking" — a false sentence about a round played last night.
...
- **The local hour is computed in SQL from `started_at` and `tz_offset`, never from a UTC hour bucket.** A UTC hour spans two different local hours in every half-hour zone (India +5:30, Nepal +5:45, Newfoundland −3:30, Chatham +12:45), so no arithmetic on the bucket can say which one the clock showed, and the version that tried put every one of those players an hour early. `started_at` still has the minutes; shift it and read the hour off. The fold takes a `local_hour` and places it, and that is all it does with it.
- **The bar's hour axis opens at 20:00, not at midnight.** All 24 hours are still drawn, and hours the door is shut are shaded rather than removed. A midnight-first axis cut the bar's own seven hours in half and parked the two pieces at opposite ends of the chart, which is the one shape the panel exists to show.
...
- **That denominator is *eligibility*, and two bounds are what make it one.** Both were missing at first and between them the headline was reading eligibility as refusal. (1) **`play_date >= NIGHT_EPOCH_DATE`** — lunch has been served since `EPOCH_DATE` and the bar opened weeks later, so every Special finished in between was a device that could not have walked through a door that was not there. (2) **Nights from ET today forward are censored**, reported as `pending` beside the rate and never inside it: a device that finished lunch at noon is not a no-show at a bar that opens at eight, and a denominator you are still filling drags every rate it touches down. Same posture as `RETENTION_WINDOW_DAYS`. Any new rate on this tab has to answer both questions before it is printed.
- **A device at the bar with no Special on the same key is counted apart, never dropped.** `barOnly` is ordinary across two devices (lunch on a phone, a drink on a laptop) and impossible through the front door, so it belongs in neither half of the rate — and a rising one is the tell that the rate measures devices where the sentence above it says people.
- **A round started outside 20:00–03:00 is drawn where it landed and counted as `outsideHours`.** The door cannot open then, so it is a wound-forward clock rather than a late drinker. Placing it is honest; explaining it away in the shape is not.
...
worker/variety.ts     the autofill's pick: rest tier, then the clash penalty against the days around it
...
shared/build.ts       the build marker's wording (BuildInfo → version / label / title). Takes the info as an
...
- **Archive** unlocks once today's Special is finished; the calendar shows every puzzle EPOCH→today with per-day status. Rounds persist per-date in `lunch-special:archive`, and **a finished Special lands there too** (`saveRound`), because the daily slot `lunch-special:round` is overwritten every day and the calendar reads only the archive (#214). Finished only: an unfinished daily resumed from the calendar would complete a daily analytics row days late. Days from before that fix come back once per device from `POST /api/rounds/past` into `lunch-special:recovered` (#216): **outcomes, never boards**, so they never go into the archive (a finished round with no guesses draws an empty grid), an archive board always outranks one, and opening a recovered day starts an ordinary replay. It is the one route on the beacon router that reads, and it answers with nothing that browser didn't send. **`RECOVER_THROUGH` (worker/pastrounds.ts) caps it** at the release that carried #215 and must never sit earlier than that day, or the days in between are lost. The route is temporary: its removal is tracked in its own issue, and that removal keeps the `lunch-special:recovered` read. See `src/game/ArchiveModal.tsx` + archive.ts + storage.ts.
...
- **A chart's axis tick and value label never sit in the flow of a fixed-height column.** Both are absolutely positioned, with `padding` on the track reserving their room (`.night-hours`, `.hourly`). In the flow they are flex siblings of the bar competing for the column's fixed height, so an hour that happened to print a label drew a *shorter* bar than an identical hour that did not — the chart encoding which labels were drawn instead of how many people played. Measured at 73px against 84px on the same value.
...
- **Autofill picks for variety, and the board says what the week holds.** `worker/variety.ts` (tested) ranks the pool by rest *tier* (never-served or ≥180 days, else less), then by a clash penalty against what the day sits next to (same region as yesterday, same country inside 14 days, two non-entrée courses back to back; a course the last ten days went without earns a bonus), then exact rest, then id. Each booking is fed back so the next day sees it. `boardBalance` / `balanceLine` in `shared/schedule.ts` print the next seven booked days' mix above the rows. **Informational, never blocking** — same rule as the rest note.
...
- Schedule: past dates locked; DELETE dish blocked while scheduled today/future; autofill = rest tier → variety penalty → exact rest (`worker/variety.ts`), skips dishes used in last 60 days
...
- **The build marker.** Every page of the game carries the version it was built from as the footer's last line, under the Privacy · Terms · Press links (`v1.7.0`, a trailing `*` for a dirty tree). **The byline is Jacob's line and nothing else goes on it**, so a screenshot or a screen recording says for itself which build it happened on. **Always on, everywhere, including production** — a marker you have to remember to switch on is one that isn't in the shot you needed it in. Four things hold: (1) it is **in the page flow, never a fixed badge** — the fixed bottom-right pill it started as covered the bottom of the check on small iPhones and ate a third of the footer's width; (2) the player sees the **version, never the sha and never the branch** (`buildVersion`, off `BuildInfo.version`, which is the nearest `v*` tag behind the commit whatever branch it was built from — `main` went out on production's footer when a deploy was dispatched by hand, and a build with no tag behind it says `dev`); the full sha and the branch ride on the span's `title` and on the admin line, because a release tag resolves to one commit and the admin is where "did that deploy land" gets asked; (3) it is **not a round mode and not a setting** — nothing about it reaches the server, the beacons or localStorage, and there is no toggle to plumb; (4) the admin carries the full label (`v1.7.0 · c61d712`) as a footer at the bottom of every screen, never in the nav, which wraps on a narrow window and put it somewhere different on each panel
- **`__BUILD__` is injected by `define` in vite.config.ts**, read from `GITHUB_SHA`/`GITHUB_REF_NAME` first and git second. CI's ordering is load-bearing: a default `actions/checkout` is shallow and carries no tags, so `git describe` there would fail or name the wrong thing. **`GITHUB_REF_NAME` is only taken as the version when it looks like a tag** — on a `workflow_dispatch` it is the branch — and otherwise `git describe --tags --match v*` finds the nearest release, which is why `deploy.yml` checks out with `fetch-depth: 0`. **No git at all is a supported state** and reads as `dev`. `define` does not run under vitest, which is why every fold in `shared/build.ts` takes a `BuildInfo` argument — **don't let one reach for the global**, and don't reference `__BUILD__` from `worker/` (it's declared for the app project only, in src/vite-env.d.ts). Under `npm run dev` the value is fixed at server start
