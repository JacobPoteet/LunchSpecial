...
- **The band carries navigation, not a sentence**, which makes it the only one that does. A stranger needs two things they can't guess: how to reach the end without playing six guesses, and where the real game is. `npm run a11y` scans it as its own state — a button and a link on mustard is contrast work that exists nowhere else.

**The demo never reaches `/admin`.** A read-only back office was built for it and then removed before it shipped, and the reasoning is worth keeping because the idea will come back:

- **The dashboard is the densest work in this repo** — fourteen pure folds, seven tabs, the small-sample rules — and it is the part nobody outside can be shown. That is a real cost, and it is not enough.
- **A permanently public admin URL is the wrong shape.** `/demo` is permanent because it goes on a résumé; nobody puts an admin dashboard on one. If the back office is ever shown again it should be a **signed, expiring link you mint and hand out**, like the showcase link, not a route anyone can find.
- **Gate reads with an ALLOW list, not a deny list.** The removed version denied a named set of paths, which meant every admin GET added afterwards would have been public in the demo until somebody remembered otherwise. On a security boundary the default has to be private; the convenience of a new panel appearing by itself is not worth it.
- **Two defects surfaced in the two days it existed**, and only one was a leak: it served the forward schedule and every unserved dish's clue text (fixed), and `?demo=1` kept an existing full session, so the owner — the only person who ever checks that URL — was the only person it never showed the demo to. **A preview that shows you something other than what it previews is worse than no preview**, and it is how something unsafe gets shipped by someone who checked.

**Anything that ever exposes prod data publicly has to answer this first:** `/api/stats/breakdown` already serves the headline counts, the guess distribution, the funnel, the whole growth series and the country table with no auth and an open CORS header, because that is what feeds the README badges. Masking numbers anywhere else is theatre while that is true — and a digit mask leaks the order of magnitude it hides, chart geometry survives it, and it would bury the Wilson intervals and censored denominators that exist *because* the samples are thin.
