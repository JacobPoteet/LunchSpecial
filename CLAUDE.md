...
- **Growth chart** (admin Trends tab, the first panel): games played per ET day since the first round ever recorded, every kind together, with a least-squares trend line through it (GitHub #90). It rides `AnalyticsSummary.growth` and is folded from the **same all-time hour buckets** `hourly`/`activeDates` already read, so it costs no extra query — pure in `worker/growth.ts` (`foldGrowth`, unit-tested in growth.test.ts), drawn by `GrowthChart` in TrendsPanel.tsx. Five things are load-bearing:
  1. **All-time, not the `daily` series.** `daily` stops ~5 weeks back; growth is exactly the thing a moving window can't show. It's the one chart that isn't a window, which is why it sits above the 30-day spark rather than replacing it — the spark answers "what are they playing", this answers "are more people playing".
  2. **Quiet days are zeros, not gaps.** The series is zero-filled server-side from the first recorded round through today, so the x-axis is calendar time. Dropping empty days would let a dead week close up and read as continuous play — and it would tilt the fit, since the regression is over the filled series.
  3. **The fit is clipped, never clamped.** A falling trend line legitimately leaves the plot below zero; pinning it to the axis would flatten the slope it exists to show (`clipPath` in the SVG). Same reason `growthNote()` drops the fitted endpoints from its sentence when either is negative — a fit isn't a count.
  4. **Quoted per week, withheld under a week of history.** The slope of a game this size is a fraction of a game per day, so "+0.4" reads as nothing; the headline says games *per week*. No line at all under `GROWTH_TREND_MIN_DAYS` (7) — the same rule `paceNote` follows for a thin baseline — and a real flat run says "holding steady" rather than dressing sub-`GROWTH_FLAT_PER_WEEK` noise with an arrow.
  5. **The trend line is dashed ink, not a series hue.** It's an annotation over the data, not a second measurement, so it takes neither the kind palette nor the event palette. The measured line is teal (the event palette's "start") with a same-hue area fill — mass under the curve, not a second value.
