...
This file is that brief, written out. It exists because 26 batches drifted a long way apart: the
first fifty dishes average 315 characters across their five clues, the last fifteen average 784,
and about half the catalogue has a fifth clue that only repeats the fourth.

Read it before writing a clue, in `/admin` or in a `migrations/000N_add_*.sql`.
...
work. Beat 1 currently gives the country outright for 90 dishes, and does it from a template: six
clues contain "a very large south asian country", six more "an east asian island nation", four "a
country south of the United States". A player who does this daily learns the decoder ring in a
fortnight.
...
likely to go soft: 77 beat-3 clues open "It is the…" or "It's the…", and 52 combine praise words
with no fact at all.
...
Beat 4 is almost entirely passive today ("Fried tortilla chips are smothered with…", "A tin of
...
in front of you.** Neither wants a shorter version of beat 4, which is what 186 dishes have now.
...
`worker/data-integrity.test.ts` for any dish listed in its `MIGRATED` set.
...
4. **A translation of the name is allowed at beat 5 and nowhere else.** "Rancher's eggs" is not
   ...
   beat that gives the country away anyway. Huevos Rancheros currently spends it on beat 3.
...
8. **No phrase of five or more words appears on two different dishes.** 31 do today.
...
## 5. Two tests before you commit a dish
...
51 clues currently assert a specific year and nothing checks any of them.
