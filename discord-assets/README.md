...
cloche/favicon mark, and the OFL fonts) — nothing new was drawn. Regenerate any time with:
...
| `cover-art.png` | **Activities → Art Assets → Cover Art** | ≥1024w, 16:9 (also crops to 13:11) — has the title | diner backdrop + neon "Lunch Special" wordmark |
...
- `build.mjs` — composites/rasterizes via `sharp`; writes a temp `fonts.conf` (gitignored) so
  librsvg finds the repo's Yellowtail / Alfa Slab One fonts by family name.
