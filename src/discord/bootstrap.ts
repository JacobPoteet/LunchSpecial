...
// the web (localStorage state, no accounts). Three things use the SDK: Rich
// Presence (src/discord/presence.ts), outbound links (src/discord/links.ts) and
// the portrait lock below. Only the first takes OAuth, including the `identify`
// scope its handshake turned out to require, and even there the user object is
